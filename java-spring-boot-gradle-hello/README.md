...
## Spring Boot + Gradle — ข้อดีและข้อเสีย
...
| **Gradle build เร็วกว่า Maven** | Gradle ใช้ incremental build + build cache ทำให้ build ซ้ำเร็วขึ้นมาก โดยเฉพาะ project ใหญ่ |
| **Groovy/Kotlin DSL** | build.gradle เขียนเป็น code ได้ ยืดหยุ่นกว่า XML ของ Maven, ทำ custom task ง่าย |
...
| **Gradle learning curve** | Gradle DSL ยืดหยุ่นแต่ซับซ้อนกว่า Maven XML, debug build script ยากกว่า |
| **Gradle daemon ใช้ memory** | Gradle daemon ใช้ memory 500MB+ สำหรับ local development |
...
Java microservice built with [Spring Boot 3.4](https://spring.io/projects/spring-boot) + [Gradle](https://gradle.org/) that returns a sample loan-service JSON log entry.

## Prerequisites

- [Docker](https://www.docker.com/) (recommended)
- [Java](https://adoptium.net/) >= 21 (optional, for local development)
- [Gradle](https://gradle.org/) >= 8.x (optional, for local development)
...
# Custom port / config
PORT=9090 SERVICE_ENV=staging LOG_LEVEL=DEBUG gradle bootRun
```
...
```bash
# Build image
docker build -t java-spring-boot-gradle-hello .

# Run container
docker run -p 8080:8080 java-spring-boot-gradle-hello
...
```
java-spring-boot-gradle-hello/
├── build.gradle                                     # Factor 2 (dependencies)
...
- Source code อยู่ใน Git repository เดียว (`microservice/java-spring-boot-gradle-hello`)
- Push ไปยัง GitHub: `theerayut99/aj-jojo-microservice-homework-day1`

### 2. Dependencies — Explicitly declare and isolate dependencies

- ประกาศ dependencies ทั้งหมดใน `build.gradle` (Gradle) พร้อม version ชัดเจน
- Spring Boot plugin + dependency-management plugin จัดการ dependency versions ให้เข้ากันอัตโนมัติ
...
- **Build**: `gradle build` หรือ Docker multi-stage build (`gradle:8-jdk21-alpine` → compile)
- **Release**: Docker image (`java-spring-boot-gradle-hello:latest`) รวม JRE + JAR
- **Run**: `docker run -p 8080:8080 java-spring-boot-gradle-hello`
...
- Procfile ประกาศ process type: `web: java -jar build/libs/*.jar`
